...
class ConfigManager:
    ...
    def load_json(self, file_key, default_data=None):
        ...
        if os.path.exists(path) and os.path.getsize(path) > 0:
            try:
                ...
                log.warning("File JSON loi/hong, dung du lieu mac dinh: %s (%s)", path, e)
    ...
    def get_existing_skus(self):
        try:
            ...
            log.warning("Khong doc duoc danh sach SKU tu %s: %s", self.base_path, e)
            return []
    ...
    def get_all_profiles(self):
        try:
            ...
            log.warning("Khong doc duoc danh sach profiles tu %s: %s", self.profiles_dir, e)
            return []
    ...
    def get_safety_account(self, exclude_list=None):
        ...
        log.warning("Khong con nick nao ranh hoac con token cho ngay hom nay!")
        return None
